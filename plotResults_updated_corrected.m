...
function stats = plotResults_updated(fName, plot_all_angles)
    ...
    % Safety: if mask accidentally excludes everything, don't filter
    if ~any(valid_mask)
    ...
    if plot_all_angles
        plot_cdf(abs(phi_err(valid_mask)),   '|Δφ| (Euler)');
        plot_cdf(abs(psi_err(valid_mask)),   '|Δψ| (Euler)');
    end
