...
function stats = plotResults_updated_thetaOnly_perfRuntime(fName, plot_all_angles)
    ...
    abs_theta = abs(theta_err);   % <-- ADD THIS LINE
% --- 6. Plotting (Meaningful Plot Pack) ---
    ...
    % ---------- Figure H: Combined Performance & Runtime ----------
    % One compact figure to inspect accuracy + execution time behavior together.
    figH = figure('Name', 'Performance & Runtime (Theta-only)', 'NumberTitle', 'off');
    tiledlayout(figH, 2, 2, 'Padding', 'compact', 'TileSpacing', 'compact');

    % (1) CDF of 3D translation error (Estimated)
    nexttile; hold on; grid on;
    plot_cdf(e3D_est(valid_mask), '||\Delta T|| (m)');
    title('CDF: Translation error');

    % (2) CDF of |theta| error
    nexttile; hold on; grid on;
    plot_cdf(abs_theta(valid_mask), '|\Delta \theta| (deg)');
    title('CDF: Theta error');

    % (3) Execution time vs sample index
    nexttile; grid on;
    if any(duration(valid_mask) > 0)
        plot(sampleNo(valid_mask), duration(valid_mask), '-o', 'LineWidth', 1.2);
        xlabel('Sample'); ylabel('Duration (s)');
        title('Runtime per sample');
    else
        text(0.1, 0.5, 'duration not available (all zeros / missing)', 'Units','normalized');
        axis off;
    end

    % (4) Execution time vs translation error
    nexttile; grid on;
    if any(duration(valid_mask) > 0)
        scatter(e3D_est(valid_mask), duration(valid_mask), 20, 'filled', 'MarkerFaceAlpha', 0.5);
        xlabel('||\Delta T|| (m)'); ylabel('Duration (s)');
        title('Runtime vs translation error');
    else
        text(0.1, 0.5, 'duration not available (all zeros / missing)', 'Units','normalized');
        axis off;
    end
